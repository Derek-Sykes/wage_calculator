...
import {getUser, createUser, deleteUser, updateUser, getUsersJson} from '../db.js'
...
Router.post('/register', redirectHome, async (req, res) => {
    ...
    if(fname && lname && email && username && password){
        ...
        if (!exists && isSecure(password)){
            ...
            // not secure
            message = 'Password must be at least 8 characters and include atleast 1 uppercase letter and number'
            return res.redirect(`/register?message=${encodeURIComponent(message)}`)
        }
    }else{
